...
class Model(baseline.Model):
    def __init__(self,
        ...
        self.context_linear = nn.Linear(4 * num_heads * hidden_size, 4 * num_heads * hidden_size)
        self.question_linear = nn.Linear(4 * num_heads * hidden_size, 4 * num_heads * hidden_size)
        self.relu = nn.ReLU()
    ...
    def forward(self,
        ...
        batch_size, num_words, num_chars = context_char_idxs.size()
        device = context_char_idxs.device

        # pad for enumeration
        context_glove_idxs = torch.cat(
            [context_glove_idxs, torch.zeros(batch_size, self.max_ans_len - 1).long().to(device)],
            1)
        context_word_idxs = torch.cat(
            [context_word_idxs, torch.zeros(batch_size, self.max_ans_len - 1).long().to(device)], 1)
        context_char_idxs = torch.cat(
            [context_char_idxs, torch.zeros(batch_size, self.max_ans_len - 1, num_chars).long().to(device)], 1)
        ...
        q_stack = torch.cat([q1, q2], 1)
        q_stack = self.relu(self.question_linear(q_stack))
        ...
        for i in range(self.max_ans_len):
            x2_list.append(x2[:, i:i+num_words, :])
